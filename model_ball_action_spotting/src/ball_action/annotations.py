...
def prepare_game_spotting_results(half2class_actions: dict, game: str, prediction_dir: Path):
    ...
    # Zaman siralamasi (dakika*60+sanisi)
    results_spotting["predictions"].sort(
        key=lambda x: int(x["gameTime"].split(":")[0]) * 60
                          + int(x["gameTime"].split(":")[1])
    )

    # JSON'u ball_results_spotting.json olarak kaydet
    results_path = game_prediction_dir / "ball_results_spotting.json"
    with open(results_path, "w") as outfile:
        json.dump(results_spotting, outfile, indent=4)
    print("Ball results spotting kaydedildi:", results_path)

    # postprocess parametrelerini kaydet
...
def save_npz(raw_predictions_path: Path | str,
             frame_indexes: list[int],
             raw_predictions: np.ndarray) -> None:
    """
    Guvenli sekilde .npz dosyasi kaydeder:
      - Klasor yoksa olusturur
      - Dosyayi diskten yazip flush ve fsync yapar
    """
    raw_predictions_path = Path(raw_predictions_path)
    os.makedirs(raw_predictions_path.parent, exist_ok=True)
    with open(raw_predictions_path, "wb") as f:
        np.savez(f,
                 frame_indexes=frame_indexes,
                 raw_predictions=raw_predictions)
        f.flush()
        os.fsync(f.fileno())
    print(f"NPZ kaydedildi: {raw_predictions_path}")
