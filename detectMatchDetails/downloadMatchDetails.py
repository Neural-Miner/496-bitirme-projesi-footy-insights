# Bein sports'tan indirdigimiz 2011-2012 sezonundan itibarenki Super Lig videolarina karsilik mac kadrosu bilgisini iceren
# json dosyalarini TFF sitesinden indirmeyi saglar.
        ...
        if parentTable:
            ...
            for row in rows:
                ...
                if len(cells) == 3:  # Ensure it's a valid match row
                    ...
                    # Ilgili mac icin bilgiler alinacak, fonksiyon cagir

                    getMatchLineup(matchLink, headers, firstTeamExtracted, secondTeamExtracted, season, weekText)





def getMatchLineup(matchLink, headers, firstTeam, secondTeam, season, week):
    ...
    extract_team_data(soupObject, "takim_1", "grdTakim1", firstTeam, macDetaylari)

    # Extract data for team 2
    extract_team_data(soupObject, "takim_2", "grdTakim2", secondTeam, macDetaylari)

    # Her sezon icin ayri klasor
    season_folder = os.path.join("./mac_verileri", season)

    if not os.path.exists(season_folder):
        os.makedirs(season_folder)

    file_path = os.path.join(season_folder, f"{season}_{week}_{firstTeam}_{secondTeam}.json")
